...
void
vofi_get_side_intersections(integrand impl_func, vofi_real fe[],
                            vofi_creal x0[], vofi_real lim_intg[],
                            vofi_creal stdir[], vofi_creal h0,
                            vofi_int_cpt nsub)
{
    int f_iat;
    ...
    if (fe[0] * fe[1] < 0.0) {
        dh0 =
            vofi_get_segment_zero(impl_func, fe, x0, stdir, h0, true_sign);
        if (fe[0] > 0.0)
            dh0 = h0 - dh0;
        lim_intg[*nsub] = dh0;
        (*nsub)++;
        ...
        if (f_iat != 0) {
            xfsa =
                vofi_get_segment_min(impl_func, fe, x0, stdir, h0, f_iat,
                                     max_iter);
            if (xfsa.iat != 0) {
                fh0 = fe[1];
                fe[1] = xfsa.fval;
                dh0 =
                    vofi_get_segment_zero(impl_func, fe, x0, stdir,
                                          xfsa.sval, true_sign);
                ...
                fe[1] = fh0;
                dh0 =
                    vofi_get_segment_zero(impl_func, fe, xfsa.xval, stdir,
                                          ss, true_sign);
                if (fe[0] > 0.0 || fe[1] < 0.0)
                    dh0 = ss - dh0;
...
void
vofi_get_face_intersections(integrand impl_func, min_data xfsa,
                            vofi_creal x0[], vofi_real lim_intg[],
                            ...
                            vofi_creal h0, vofi_int_cpt nsub)
{
    ...
    vofi_real pt0[NDIM], pt1[NDIM], pt2[NDIM], ptt[NDIM], mp0[NDIM],
        mp1[NDIM];
    vofi_real ss[NDIM], exdir[NDIM], indir[NDIM], fe[NEND];
    vofi_real ss0, ds0, fpt0, sss, sst, ssx, ssy, tol2, normdir, d1, d2,
        a1, a2;
    ...
    for (k = -1; k <= 1; k = k + 2) {
        ...
        while (not_conv && iter < max_iter) {   /* iterative loop for the limit */
            ...
            if (fe[1] > 0.) {
                ds0 =
                    vofi_get_segment_zero(impl_func, fe, mp1, exdir, sst,
                                          f_iat);
                sst = ds0;
            }
            /* DEBUG 6 */
            ...
            if (ipt) {
                ...
                if (fe[0] * fe[1] < 0.) {   /* get other zero along secondary dir */
                    ds0 =
                        vofi_get_segment_zero(impl_func, fe, ptt, indir,
                                              sss, f_iat);
                    for (i = 0; i < NDIM; i++)
