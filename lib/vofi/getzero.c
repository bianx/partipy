...
vofi_real
vofi_get_segment_zero(integrand impl_func, vofi_creal fe[],
                      vofi_creal x0[], vofi_creal dir[], vofi_creal s0,
                      vofi_cint f_sign)
{
    int not_conv, iss, i, iter;
    vofi_cint max_iter = 200;
    vofi_real xs[NDIM], sl, sr, ss, sold, fl, fr, fs, fold, dss, dsold,
        dfs, sz;
    ...
    while (not_conv && iter < max_iter) {   /* iterative loop */
        ...
        if (((ss - sr) * dfs - fs) * ((ss - sl) * dfs - fs) > 0.0 || fabs(2. * fs) > fabs(dsold * dfs)) {   /* bisection step */
            dsold = dss;
            sold = ss;
            dss = 0.5 * (sr - sl);
            ss = sl + dss;
