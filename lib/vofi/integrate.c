...
double
vofi_get_area(integrand impl_func, vofi_creal x0[],
              vofi_creal int_lim_intg[], vofi_creal pdir[],
              vofi_creal sdir[], vofi_creal h0, vofi_cint nintsub,
              vofi_cint nintpt)
{
    ...
    for (ns = 1; ns <= nintsub; ns++) { /* loop over the rectangles */
        ...
        if (!cut_rect) {        /* no interface: full/empty rectangle */
            ...
            for (k = 0; k < npt; k++) {
                ...
                fe[1] = impl_func(x21);
                if (fe[0] * fe[1] < 0.)
                    ht = vofi_get_segment_zero(impl_func, fe, x20, pdir,
                                               h0, true_sign);
...
double
vofi_get_volume(integrand impl_func, vofi_creal x0[],
                vofi_creal ext_lim_intg[], vofi_creal pdir[],
    ...
    for (ns = 1; ns <= nextsub; ns++) { /* loop over the rectangular hexahedra */
        ...
        if (!cut_hexa) {        /* check lower side along secondary direction */
            ...
            if (fe[0] * fe[1] <= 0.)
                ...
                f_iat =
                    vofi_check_side_consistency(impl_func, fe, x1, sdir,
                                                h0);
                if (f_iat != 0) {
                    xfsa =
                        vofi_get_segment_min(impl_func, fe, x1, sdir, h0,
                                             f_iat, max_iter);
                    cut_hexa = xfsa.iat;
                }
            }
        ...
        if (!cut_hexa) {        /* check upper side along secondary direction */
            ...
            if (fe[0] * fe[1] <= 0.)
                ...
                f_iat =
                    vofi_check_side_consistency(impl_func, fe, x2, sdir,
                                                h0);
                if (f_iat != 0) {
                    xfsa =
                        vofi_get_segment_min(impl_func, fe, x2, sdir, h0,
                                             f_iat, max_iter);
                    cut_hexa = xfsa.iat;
                }
            }
        ...
        if (!cut_hexa) {        /* no interface: full/empty hexahedron */
            ...
            for (k = 0; k < nexpt; k++) {
                ...
                nintsub =
                    vofi_get_limits(impl_func, x1, int_lim_intg, pdir,
                                    sdir, tdir, h0, stdir);
                area_n =
                    vofi_get_area(impl_func, x1, int_lim_intg, pdir, sdir,
                                  h0, nintsub, nintpt);
                /* DEBUG 4 */
