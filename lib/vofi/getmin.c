...
min_data
vofi_get_segment_min(integrand impl_func, vofi_creal fe[], vofi_creal x0[],
...
min_data
vofi_get_face_min(integrand impl_func, vofi_creal x0[], vofi_creal sdir[],
                  vofi_creal tdir[], chk_data ivga, vofi_creal h0)
{
    int i, not_conv, iter, k, ipt, iss;
    vofi_cint max_iter = 50, max_iter_line = 50;
    vofi_real xs0[NDIM], xs1[NDIM], xs2[NDIM], xt1[NDIM], xt2[NDIM],
        res[NDIM], hes[NDIM];
    vofi_real rs0[NDIM], hs0[NDIM], pcrs[NDIM], nmdr[NDIM], cndr[NDIM],
        ss[NDIM], fe[NEND];
    vofi_real eps2, fp0, fs1, fs2, ft1, ft2, dfs, dft, d2fs, d2ft, mcd,
        ss0, ss1, beta;
    ...
    for (i = 0; i < NDIM; i++) {
        res[i] =
            rs0[i] + dfs * ivga.igs * sdir[i] + dft * ivga.igt * tdir[i];
    ...
    while (not_conv && iter < max_iter) {   /* iterative loop */
        ...
        xfsa =
            vofi_get_segment_min(impl_func, fe, xs0, nmdr, ss0, ivga.iat,
                                 max_iter_line);
