...
int
vofi_get_limits(integrand impl_func, vofi_creal x0[], vofi_real lim_intg[],
                vofi_creal pdir[], vofi_creal sdir[], vofi_creal tdir[],
                vofi_creal h0, vofi_cint stdir)
{
    ...
    if (stdir == 2) {           /* get the internal limits along sdir */
        for (j = 0; j < 2; j++) {   /* two sides */
            ...
            fe[0] = impl_func(x1);
            fe[1] = impl_func(x2);
            vofi_get_side_intersections(impl_func, fe, x1, lim_intg, sdir,
                                        h0, &nsub);
        }
        ...
        for (k = 0; k < 2; k++) {   /* two planes */
            ...
            for (j = 0; j < 2; j++) {   /* two sides */
                ...
                vofi_get_side_intersections(impl_func, fe, x1, lim_intg,
                                            tdir, h0, &nsub);
                /* DEBUG 3 */
            }
            if (nvp == 4 || nvn == 4) { /* get the extra limits in the face */
                ...
                fvga =
                    vofi_check_face_consistency(impl_func, fv, x1, sdir,
                                                tdir, h0);
                if (fvga.iat != 0)
                    xfsa =
                        vofi_get_face_min(impl_func, x1, sdir, tdir, fvga,
                                          h0);
                if (xfsa.iat != 0)
                    vofi_get_face_intersections(impl_func, xfsa, x1,
                                                lim_intg, sdir, tdir, h0,
                                                &nsub);
                /* DEBUG 5 */
            }
        }
