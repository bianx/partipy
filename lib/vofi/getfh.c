...
vofi_real
vofi_Get_fh(integrand impl_func, vofi_creal x0[], vofi_creal h0,
            vofi_cint ndim0, vofi_cint ix0)
{
    int i, k, isw;
    ...
    vofi_real x1[NDIM], x2[NDIM], xn1[NDIM], xn2[NDIM], der[NDIM],
        fe[NEND];
    ...
    if (isw) {
        ...
        if (k < kmax) {
            ...
            if (dd < EPS_M) {
                fprintf(stderr,
                        "WARNING: the zero is almost a critical point:  \n");
                fprintf(stderr,
                        "(x,y,z): (%e, %e, %e) |f|,|grad(f)|: %e, %e \n",
