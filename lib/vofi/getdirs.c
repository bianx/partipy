...
dir_data
vofi_get_dirs(integrand impl_func, vofi_creal x0[], vofi_real pdir[],
              vofi_real sdir[], vofi_real tdir[], vofi_creal h0,
                ...
                for (j = 0; j < NLSY; j++) {
                    if (fabs(f0[k][i][j]) <= fh) {
                        ...
                        for (n = 0; n < ndim0; n++) {
                            ...
                            xm[n] -= dh;
                            df0[k][i][j][n] =
                                0.5 * (impl_func(xp) - impl_func(xm)) / dh;
        ...
        for (k = 0; k <= kmax; k++)
            for (i = 0; i < NLSX; i++)
                for (j = 0; j < NLSY; j++) {
                    if (fabs(f0[k][i][j]) <= fh) {
                        denom =
                            Sq(df0[k][i][j][jp]) + Sq(df0[k][i][j][js]) +
                            EPS_NOT0;
