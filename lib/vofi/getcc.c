...
vofi_real
vofi_Get_cc(integrand impl_func, vofi_creal x0[], vofi_creal h0,
            vofi_creal fh, vofi_cint ndim0)
{
    int nsub;
    ...
    if (icps.icc >= 0)
        ...
        nsub =
            vofi_get_limits(impl_func, x0, side, pdir, sdir, tdir, h0,
                            ndim0);
        if (ndim0 == 2)
            cc = vofi_get_area(impl_func, x0, side, pdir, sdir, h0, nsub,
                               icps.ipt);
        else
            cc = vofi_get_volume(impl_func, x0, side, pdir, sdir, tdir, h0,
                                 nsub, icps.ipt);
    }
