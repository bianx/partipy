...
double
f0(unsigned dim, const double *x, void *params)
{
    double prod = 1.0;
    unsigned int i;
...
double
f1(unsigned dim, const double *x, void *params)
{
...
/* double gaussian */
double
f2(unsigned dim, const double *x, void *params)
{
    double a = *(double *) params;
    double sum1 = 0.;
    double sum2 = 0.;
...
/* Tsuda's example */
double
f3(unsigned dim, const double *x, void *params)
{
...
static double
morokoff(unsigned dim, const double *x, void *params)
{
    double p = 1.0 / dim;
...
int
f_test(unsigned dim, const double *x, void *data_,
       unsigned fdim, double *retval)
{
    ...
    for (j = 0; j < fdim; ++j) {
        double fdata =
            which_integrand[j] == 6 ? (1.0 + sqrt(10.0)) / 9.0 : 0.1;
...
static double
exact_integral(int which, unsigned dim, const double *xmax)
{
    unsigned i;
    double val;
...
main(int argc, char **argv)
{
    ...
    cubature(integrand_fdim, f_test, NULL,
             dim, xmin, xmax, maxEval, 0, tol, ERROR_INDIVIDUAL, val, err);
    for (i = 0; i < integrand_fdim; ++i) {
        printf
            ("integrand %d: integral = %0.11g, est err = %g, true err = %g\n",
