...
static double
errMax(unsigned fdim, const esterr * ee)
{
    double errmax = 0;
    unsigned k;
...
static hypercube
make_hypercube(unsigned dim, const double *center, const double *halfwidth)
{
    unsigned i;
    hypercube h;
...
static hypercube
make_hypercube_range(unsigned dim, const double *xmin, const double *xmax)
{
...
static region
make_region(const hypercube * h, unsigned fdim)
{
    region R;
...
static int
alloc_rule_pts(rule * r, unsigned num_regions)
{
...
static rule *
make_rule(size_t sz,            /* >= sizeof(rule) */
...
static int
eval_regions(unsigned nR, region * R, integrand_v f, void *fdata, rule * r)
{
    unsigned iR;
 ...
 */
static void
evalR_Rfs(double *pts, unsigned dim, double *p, const double *c,
          const double *r)
{
    unsigned i;
...
static void
evalRR0_0fs(double *pts, unsigned dim, double *p, const double *c,
            const double *r)
{
    unsigned i, j;
...
static void
evalR0_0fs4d(double *pts, unsigned dim, double *p, const double *c,
...
static void
destroy_rule75genzmalik(rule * r_)
{
...
static int
rule75genzmalik_evalError(rule * r_, unsigned fdim, integrand_v f,
...
#define VALS(i) v[fdim*(i)]
        for (iR = 0; iR < nR; ++iR) {
            ...
            result =
                R[iR].h.vol * (r->weight1 * val0 + weight2 * sum2 +
                               r->weight3 * sum3 + weight4 * sum4 +
                               r->weight5 * sum5);
            res5th =
                R[iR].h.vol * (r->weightE1 * val0 + weightE2 * sum2 +
...
static rule *
make_rule75genzmalik(unsigned dim, unsigned fdim)
{
    rule75genzmalik *r;

    if (dim < 2)
        return NULL;            /* this rule does not support 1d integrals */
...
static rule *
make_rule15gauss(unsigned dim, unsigned fdim)
{
    if (dim != 1)
        return NULL;            /* this rule is only for 1d integrals */
...
static void
heap_resize(heap * h, size_t nalloc)
{
    h->nalloc = nalloc;
    if (nalloc)
        h->items =
            (heap_item *) realloc(h->items, sizeof(heap_item) * nalloc);
    else {
...
static heap
heap_alloc(size_t nalloc, unsigned fdim)
{
    heap h;
    unsigned i;
...
static int
heap_push_many(heap * h, size_t ni, heap_item * hi)
{
    size_t i;
...
static int
converged(unsigned fdim, const esterr * ee,
...
static int
rulecubature(rule * r, unsigned fdim,
             integrand_v f, void *fdata,
             const hypercube * h,
             size_t maxEval,
             double reqAbsError, double reqRelError,
             error_norm norm, double *val, double *err, int parallel)
{
    size_t numEval = 0;
    heap regions;
    unsigned i, j;
    ...
    if (fdim <= 1)
        norm = ERROR_INDIVIDUAL;    /* norm is irrelevant */
    if (norm < 0 || norm > ERROR_LINF)
        return FAILURE;         /* invalid norm */
...
static int
cubature(unsigned fdim, integrand_v f, void *fdata,
...
int
hcubature_v(unsigned fdim, integrand_v f, void *fdata,
            unsigned dim, const double *xmin, const double *xmax,
            size_t maxEval, double reqAbsError, double reqRelError,
...
int
hcubature(unsigned fdim, integrand f, void *fdata,
          unsigned dim, const double *xmin, const double *xmax,
          size_t maxEval, double reqAbsError, double reqRelError,
          error_norm norm, double *val, double *err)
{
