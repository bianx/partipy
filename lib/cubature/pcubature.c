...
static int
compute_cacheval(const unsigned *m, unsigned mi,
...
static size_t
num_cacheval(const unsigned *m, unsigned mi, unsigned dim)
{
    unsigned i;
    size_t nval = 1;
...
static unsigned
eval(const unsigned *cm, unsigned cmi, double *cval,
     const unsigned *m, unsigned md,
     unsigned fdim, unsigned dim, unsigned id, double weight, double *val)
{
    ...
    if (id == dim) {
        ...
        voff =
            eval(cm, cmi, cval, m, md, fdim, dim, id + 1, weight * 2, val);
        voff +=
            fdim * (1 << cm[id]) * 2 * num_cacheval(cm + id + 1,
                                                    cmi - (id + 1),
                                                    dim - (id + 1));
    } else {
        unsigned i;
...
static void
evals(valcache vc, const unsigned *m, unsigned md,
...
static void
eval_integral(valcache vc, const unsigned *m,
...
static int
converged(unsigned fdim, const double *vals, const double *errs,
...
int
pcubature_v_buf(unsigned fdim, integrand_v f, void *fdata,
                unsigned dim, const double *xmin, const double *xmax,
                size_t maxEval,
...
int
pcubature_v(unsigned fdim, integrand_v f, void *fdata,
            unsigned dim, const double *xmin, const double *xmax,
...
int
pcubature(unsigned fdim, integrand f, void *fdata,
          unsigned dim, const double *xmin, const double *xmax,
          size_t maxEval, double reqAbsError, double reqRelError,
          error_norm norm, double *val, double *err)
{
    ...
    d.fdata = fdata;
    memset(m, 0, sizeof(unsigned) * dim);
    ret = pcubature_v_buf(fdim, fv, &d, dim, xmin, xmax,
                          maxEval, reqAbsError, reqRelError, norm,
                          m, &buf, &nbuf, 16
