...
static double
cylinder_distance(const double p[3])
{
    enum { X, Y, Z };
...
double
vof_3d(double x, double y, double z, double r)
{
    int dim;
    double Delta, h;
...
double
vof_2d(double x, double y, double r)
{
    int dim;
    double Delta, h;
    double p[2] = { -0.5, -0.5 };
...
double
vof_cylinder(double x, double y, double z, double r, double u, double v,
             double w)
{
    int dim;
  ...
  err:
    fprintf(stderr, "%s:%d: %g %g %g %g %g %g %g\n", __FILE__, __LINE__, x,
            y, z, r, u, v, w);
    return -1;
}
